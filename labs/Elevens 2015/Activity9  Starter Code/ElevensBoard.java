...
public class ElevensBoard extends Board {
    ...
    private boolean containsPairSum11(List<Integer> selectedCards) {
        for(int i = 0; i< selectedCards.size(); i++) {
            ...
            for(int k = i + 1; k < selectedCards.size(); k++) {
                int l = selectedCards.get(k).intValue();
                if(cardAt(j).pointValue() + cardAt(l).pointValue() ==11) 
                return true;
              }
          }
          return false;
    ...
    private boolean containsJQK(List<Integer> selectedCards) {
        ...
        for(Integer i : selectedCards) {
            int j = i.intValue();
            if(cardAt(j).rank().equals("jack")) {
                jack = true;
            }
            else if (cardAt(j).rank().equals("queen")) {
                queen = true;
            }
            else if (cardAt(j).rank().equals("king")) {
                king = true;
            }
        }
